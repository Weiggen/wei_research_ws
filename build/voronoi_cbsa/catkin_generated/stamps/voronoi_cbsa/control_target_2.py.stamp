...
class UAVController:
    def __init__(self):
        rospy.init_node('control_target_2', anonymous=True)
        
        # 參數設定
        self.drone_name = 'target_2'
        self.state = State()
        self.current_pose = PoseStamped()
        self.rate = rospy.Rate(20)  # 20Hz
        
        # 圓形軌跡參數
        self.circle_radius = 3.0  # 5m 半徑
        self.circle_z = 1.2       # 3m 高度
        self.circle_center_x = 0.0
        self.circle_center_y = 0.0
        self.angular_speed = 0.06  # rad/s
    ...
    def run(self):
        try:
            rospy.loginfo("開始執行target_2控制節點...")
            # 初始化開始軌跡標記為False

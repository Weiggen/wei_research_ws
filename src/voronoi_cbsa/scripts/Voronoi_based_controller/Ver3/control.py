...
class PTZCamera():
    ...
    def RosInit(self):
        ...
        rospy.Subscriber("/iris_"+str(id)+"/mavros/local_position/pose", PoseStamped, self.AgentPosCallback)
        ...
        self.pub_vel_cmd            = rospy.Publisher("/iris_"+str(id)+"/mavros/setpoint_velocity/cmd_vel", TwistStamped, queue_size=10)       
...
if __name__ == "__main__":
    ...
    #---Set Armed & offb---# 
    state_sub = rospy.Subscriber("/iris_"+str(id)+"/mavros/state", State, callback = state_cb)

    rospy.wait_for_service("/iris_"+str(id)+"/mavros/cmd/arming")
    arming_client = rospy.ServiceProxy("/iris_"+str(id)+"/mavros/cmd/arming", CommandBool)

    rospy.wait_for_service("/iris_"+str(id)+"/mavros/set_mode")
    set_mode_client = rospy.ServiceProxy("/iris_"+str(id)+"/mavros/set_mode", SetMode)
    ...
    while(not rospy.is_shutdown()):
        if(current_state.mode != "OFFBOARD" and (rospy.Time.now() - last_req) > rospy.Duration(5.0)):
            if(set_mode_client.call(offb_set_mode).mode_sent == True):
                rospy.loginfo("/iris_"+str(id)+"OFFBOARD enabled")

            last_req = rospy.Time.now()
        else:
            if(not current_state.armed and (rospy.Time.now() - last_req) > rospy.Duration(5.0)):
                if(arming_client.call(arm_cmd).success == True):
                    rospy.loginfo("/iris_"+str(id)+"Vehicle armed")

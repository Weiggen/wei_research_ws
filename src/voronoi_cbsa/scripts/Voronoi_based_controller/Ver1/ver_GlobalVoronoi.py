...
class Voronoi2D():
    def __init__(self, map_size, grid_size, camera_num, \
        ...
        self.weighted_sum = [0 for i in range(len(self.PTZs))]
        self.voronoi_sum = [0 for i in range(len(self.PTZs))]
        self.sum = np.sum(self.event)

    def Update(self, step = 0.3, event = None, event_plt = None, target = []): 
        ...
        global_voronoi, centroids = self.globalVoronoi()

        for i in range(self.camera_num):
            self.PTZs[i].UpdateVoronoi([self.PTZs[j].FoV \
                for j in range(len(self.PTZs)) if j != i], centroids[i])
        ...
        print("Observation Quality: ",sum, "Coverage Quality: ",sum1)# + area/np.sum(np.ones(self.event.shape))

        self.map.Update(map_plt, self.PTZs, self.event_plt, target, global_voronoi, centroids)
        ...
        for i in range(self.camera_num):
            ...
            self.PTZs[i].UpdateState(info, step, centroids[i])

        return cnt, self.PTZs
    ...
    def globalVoronoi(self):
        global_voronoi = np.zeros(self.event.shape)
        ideal_capacity = 380
        self.weighted_sum = [0 for i in range(len(self.PTZs))]
        self.voronoi_sum = [0 for i in range(len(self.PTZs))]
        for x in range(self.event.shape[0]):
            for y in range(self.event.shape[1]):
                distance = np.inf
                for i in range(len(self.PTZs)):
                    pos = self.PTZs[i].pos + self.PTZs[i].R*cos(self.PTZs[i].alpha)*self.PTZs[i].perspective
                    pos_self = self.PTZs[i].pos
                    # pos_self = self.PTZs[i].pos
                    if self.initialized == True:
                        r = self.PTZs[i].R*cos(self.PTZs[i].alpha)/self.grid_size[0]
                        capacity = (ideal_capacity - self.PTZs[i].coverage_quality) / ideal_capacity
                        tmp = ((self.norm(pos/self.grid_size - np.array([x,y])))**2 - (r**2)*(1 + capacity))*self.event[x, y]  
                    else:
                        tmp = ((self.norm(pos/self.grid_size - np.array([x,y])))**2)*self.event[x, y]
                    if tmp < distance:
                        distance = tmp 
                        global_voronoi[x, y] = i    
                id =  global_voronoi[x, y].astype(np.int64)
                x_p = self.PTZs[id].pos - np.array([x,y])
                g = (((self.norm(x_p)**self.PTZs[id].lamb)*(self.PTZs[id].R*cos(self.PTZs[id].alpha)\
                        - self.PTZs[id].lamb*( self.norm(x_p) - self.PTZs[id].R*cos(self.PTZs[id].alpha)) ))\
                            /(self.PTZs[id].R**(self.PTZs[id].lamb+1)))       
                self.PTZs[id].global_voronoi.append((x,y))
                self.weighted_sum[id] += np.array([x,y])*self.event[x,y]
                self.voronoi_sum[id] += self.event[x,y]
        centroids = [(self.weighted_sum[i]/self.voronoi_sum[i]).astype(np.int64) for i in range(len(self.weighted_sum))]
        for i in range(len(self.PTZs)):
            self.PTZs[i].score = 0
        self.initialized = True

        return global_voronoi, centroids
    ...
    class PTZcamera():
        ...
        def UpdateState(self, neighbors, step, centroid):
            centroidal_forces = self.ComputeCentroidal()

            formation_force = self.FormationControl(neighbors)
            self.TargetAssignment(centroid)
        ...
        def TargetAssignment(self, cnetroid):
            score = -np.inf
            self.target_assigned = -1
            pos = None
            ...
            if pos is not None and self.FoV[pos[0]-1, pos[1]-1] >= 0:
                self.score += 1
            else:
                self.stage = 1
        ...
        def ComputeCentroidal(self):
            ...
            if len(self.voronoi[0]) > 0:
                ...
                for i in range(len(self.voronoi[0])):
                    ...
                    norm = self.norm(x_p)
                    if norm == 0: continue
                    mu_V += ((norm**self.lamb)*self.voronoi_event[x_map,y_map] )/(self.R**self.lamb)
                    mu_Vv += ((norm**self.lamb)*self.event[x_map,y_map] )/(self.R**self.lamb)
                    v_V_t += ((x_p)/norm)*(cos(self.alpha) - \
                                    ( ( self.lamb*norm )/((self.lamb+1)*self.R)))*\
                                        ( (norm**self.lamb)/(self.R**self.lamb) )*self.voronoi_event[x_map,y_map]
                    ...
                    x_center += x*(((norm**self.lamb)*self.voronoi_event[x_map,y_map] )/(self.R**self.lamb))
                    y_center += y*(((norm**self.lamb)*self.voronoi_event[x_map,y_map] )/(self.R**self.lamb))
                
                v_V = v_V_t/mu_V
                delta_V = delta_V_t/mu_Vv
            ...
            #         x_p = np.array([x,y]) - self.pos
            #         norm = self.norm(x_p)
            #         mu_V_o += ((norm**self.lamb)*self.voronoi_event[x_map,y_map] )/(self.R**self.lamb)
            #         mu_Vv_o += ((norm**self.lamb)*self.event[x_map,y_map] )/(self.R**self.lamb)
            ...
            #                         *dist*((norm**self.lamb)/(self.R**self.lamb))\
            #                             *event[x_map,y_map]
            #         x_center_o += x*(((norm**self.lamb)*self.voronoi_event[x_map,y_map] )/(self.R**self.lamb))
            #         y_center_o += y*(((norm**self.lamb)*self.voronoi_event[x_map,y_map] )/(self.R**self.lamb))

            #     v_V_o = v_V_t_o/mu_V_o
            #     delta_V_o = delta_V_t_o/mu_Vv_o
        ...
        def UpdateVoronoi(self, FoVs, centroid):
            if self.stage == 1:
                self.voronoi_event = self.event_density(self.event, [[centroid, 3, 10, 0]], (1,1)) 
            ...
            self.observation_quality = np.sum(self.FoV*np.transpose(self.event))
            self.coverage_quality = np.sum(quality_map*np.transpose(self.event))
    ...
    class Map():
        ...
        def Update(self, map_plt, cameras, event, targets, global_voronoi, centroids):
            x_map = 0
            for x in range(0, self.window_size[0], self.blockSize):
                y_map = 0
                for y in range(0, self.window_size[1], self.blockSize):
            ...
            for i in range(len(centroids)):
                centroid = self.blockSize*centroids[i]
